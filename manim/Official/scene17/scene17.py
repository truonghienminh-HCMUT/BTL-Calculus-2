...
class ChiaNho(MovingCameraScene):
    def construct(self):
        ...
        yaxes2_label = axes2.get_y_axis_label("r")
        ...
        fxy = MathTex(r"f(x,y)").scale(1.5).next_to(axes1.c2p(a, 0), UP)
        fxy.move_to([-2.9, 3, 0])
        fxy.set_color(RED)
        fxy_copy = fxy.copy()
        ...
        line_r_a = Line(start= axes2.c2p(-1, 3), end= axes2.c2p(5, 3), color=RED, stroke_width=4)
        line_r_b = Line(start= axes2.c2p(-1, 1), end= axes2.c2p(5, 1), color=RED, stroke_width=4)
        line_alpha = Line(start= axes2.c2p(1, -1), end= axes2.c2p(1, 5), color=BLUE, stroke_width=4)
        line_beta = Line(start= axes2.c2p(3, -1), end= axes2.c2p(3, 5), color=BLUE, stroke_width=4)

        p1 = axes2.c2p(1, 3)  # Giao alpha và r_a
        p2 = axes2.c2p(3, 3)  # Giao beta và r_a
        p3 = axes2.c2p(3, 1)  # Giao beta và r_b
        p4 = axes2.c2p(1, 1)  # Giao alpha và r_b

        region_new_D = Polygon(p1, p2, p3, p4, fill_color=RED, fill_opacity=0.5, stroke_width=0)

        self.play(Create(axes1), Write(fxy), run_time=2)
        ...
        self.add(part_copy)
        self.play(fxy_copy.animate.move_to(RIGHT * 3 + UP *3), part1.animate.move_to(RIGHT * 3.05 + DOWN * 0.3), x_axes1_labels_copy.animate.move_to(RIGHT * 5.1 + UP * (-1.5)), y_axes1_labels_copy.animate.move_to(RIGHT * 1.7 + UP * 2), run_time=2)
        self.wait(1)
        self.play(Transform(fxy_copy, frphi), Transform(x_axes1_labels_copy, Textphi), Transform(y_axes1_labels_copy, yaxes2_label), Transform(left_line, line_r_a), Transform(right_line, line_r_b), Transform(angle_alpha, line_alpha), Transform(angle_beta, line_beta), Transform(region, region_new_D),
                D.animate.move_to(DOWN * 0.2 + RIGHT * 2.7), Uncreate(outer_arc), Uncreate(inner_arc), Uncreate(alpha_label_old), Uncreate(beta_label_old), rb.animate.move_to(LEFT * (-0.4) +  UP * 0.8), Transform(ra, r_a_sau), Create(alpha_label), Create(beta_label),
                run_time=2)
